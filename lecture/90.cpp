...
#ifdef mine
int n = 0, rabitCount = -1, simba = 2;
int map[26][26];
int simbaR, simbaC, simbaTime;
...
int main(){
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<=n; j++){
            ...
            else if(map[i][j] != 0) rabitCount++;
        }
    }
    BFS(simbaR, simbaC, simbaTime);
    return 0;
}
    ...
    while(!Q.empty()){
        ...
        if(map[x][y]!=0 && map[x][y] < simba.s){
            ...
            for(i=0; i<4; i++){
                ...
                Q.push(State(xx,yy,z+1)); //// 여기서 z+1의 역할이 매우매우 중요함
                ch[xx][yy] = 1;
            }
        }
    }
